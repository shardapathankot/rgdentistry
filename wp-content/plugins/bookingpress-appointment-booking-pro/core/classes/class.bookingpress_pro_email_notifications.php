			...
			if(!empty($bookingpress_appointment_data)) {
				...
				if( is_plugin_active( 'bookingpress-cart/bookingpress-cart.php') && preg_match( '/%cart_appointment_list%/', $template_content ) ){
					...
					if($bookingpress_is_cart == 1 && $bookingpress_order_id){
						...
						$bookingpress_appointment_extra_serv_data = $wpdb->get_results($wpdb->prepare("SELECT bookingpress_extra_service_details FROM {$tbl_bookingpress_appointment_bookings} WHERE bookingpress_order_id = %d ", $bookingpress_order_id), ARRAY_A);

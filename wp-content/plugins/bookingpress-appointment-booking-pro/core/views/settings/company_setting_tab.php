        ...
        <el-row type="flex" class="bpa-mlc-head-wrap-settings bpa-gs-tabs--pb__heading">
            ...
            <el-col :xs="12" :sm="12" :md="12" :lg="16" :xl="12">
                <div class="bpa-hw-right-btn-group bpa-gs-tabs--pb__btn-group">                    
                    ...
                    <?php do_action('bookingpress_company_setting_header_button'); ?>
                </div>                
            </el-col>
        </el-row>
        <div class="bpa-gs--tabs-pb__content-body">
            <el-form id="company_setting_form" :rules="rules_company" ref="company_setting_form" :model="company_setting_form"  @submit.native.prevent>
                <div class="bpa-gs__cb--item">
                    ...
                    <el-row type="flex" class="bpa-gs--tabs-pb__cb-item-row">
                        <el-col :xs="12" :sm="12" :md="12" :lg="6" :xl="6" class="bpa-gs__cb-item-left">
                            <h4> <?php esc_html_e('Company Icon', 'bookingpress-appointment-booking'); ?></h4>
                        </el-col>
                        <el-col :xs="12" :sm="12" :md="12" :lg="18" :xl="18">
                            <el-upload class="bpa-upload-component" ref="iconRef" action="<?php echo wp_nonce_url(admin_url('admin-ajax.php') . '?action=bookingpress_upload_company_icon', 'bookingpress_upload_company_icon'); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped --Reason - esc_html is already used by wp_nonce_url function and it's false positive ?>" :on-success="bookingpress_upload_company_icon_func" multiple="false" :show-file-list="comShowFileList" limit="1" :on-exceed="bookingpress_company_icon_upload_limit" :on-error="bookingpress_company_icon_upload_err" :on-remove="bookingpress_remove_company_icon" :before-upload="checkUploadedIconFile" drag>
                                <span class="material-icons-round bpa-upload-component__icon">cloud_upload</span>
                            <div class="bpa-upload-component__text" v-if="company_setting_form.company_icon_url == ''"><?php esc_html_e('jpg/png/webp files with a size less than 500kb', 'bookingpress-appointment-booking'); ?>  </div>
                            </el-upload>
                            <div class="bpa-uploaded-avatar__preview bpa-uploaded-avatar__preview--company-settings" v-if="company_setting_form.company_icon_url != ''">
                                <button class="bpa-avatar-close-icon" @click="bookingpress_remove_company_icon">
                                    <span class="material-icons-round">close</span>
                                </button>
                                <el-avatar shape="square" :src="company_setting_form.company_icon_url" class="bpa-uploaded-avatar__picture"></el-avatar>
                            </div>
                        </el-col>
                    </el-row>
                </div>

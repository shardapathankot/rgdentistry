    ...
    class bookingpress_payment_gateways Extends BookingPress_Core
    {
        ...
        function bookingpress_do_autologin(){
            ...
            if( is_user_logged_in() && !empty( $_GET['bp_tp_token_check'] ) && 'yes' == $_GET['bp_tp_token_check'] && !empty( $_GET['bp_tp_nonce'] ) && !empty( $_GET['appointment_id'] )  ){
                ...
                $entry_id = base64_decode( $_GET['appointment_id'] );
                ...
                $get_customer_id = $wpdb->get_row( $wpdb->prepare( "SELECT bookingpress_customer_id FROM {$tbl_bookingpress_customers} WHERE bookingpress_wpuser_id = %d AND bookingpress_user_status = %d AND bookingpress_user_type = %d ORDER BY bookingpress_customer_id DESC", $current_logged_in_userid, 1, 2 ) );
                ...
                if( !empty( $get_customer_id ) ){
                    ...
                    $appointment_data = $wpdb->get_var( $wpdb->prepare( "SELECT bookingpress_appointment_booking_id FROM {$tbl_bookingpress_appointment_bookings} WHERE bookingpress_customer_id = %d" . $bookingpress_appointment_data_where_clause , $customer_id ) );
